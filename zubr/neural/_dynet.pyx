...
# cdef Expression scalarInput(float s):
#     return _cg.inputValue(s)
...
cpdef Expression inputTensor(arr,batched=False):
...
cdef Expression makeTensor(arr):
    """C version of the function above"""
    arr = arr.flatten(order='F')
    return _cg.inputMatrixLiteral(arr,arr.shape,1)
