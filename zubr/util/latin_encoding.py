...
import sys
import unicodedata
from zubr.util.japanese_transliterate import JapaneseTransliterator
...
def small_encode(input):
    """Starts by converting out of cyrillic and japanese script, then reduces character
    representations to an 8bit representation (which involves removing accent marks, umlauts, etc..)

    :param input: the text input 
    """
    #b = JapaneseTransliterator(u"")
    #input = b.transliterate_from_hrkt_to_latn(text=input)
    #input = b.transliterate_from_hira_to_latn(input)
    new_word = ' '.join([__cyrillic_transliterate(w) for w in input.split()])
    input = new_word
    if not isinstance(input,text_type):
        input = text_type(input, sys.getdefaultencoding(), 'strict')
    input = unicodedata.normalize('NFKC', input)
    return input.translate(short_table)

## cyrillic translation

def __cyrillic_transliterate(word):
    converted_word = ''
    for char in word:
        transchar = ''
